							...
							<div class="ui stacked segment">
								<table id="tablenya" class="ui celled table" cellspacing="0" width="100%">
									...
									<tbody>
										<?php
										$no=1;
										foreach ($tutor as $s => $row) {
											?>
											<tr>
												<td><?=$no++;?></td>
												<td><?=$row->nama;?></td>
												<td><img width="70px" height="60px" src="<?=base_url()?>img/tutor/<?=$row->foto;?>"></td>
												<td width="15%"><?=$row->jabatan;?></td>
												<td><?=substr($row->fb, 0, 15);?>...</td>
												<td><?=substr($row->ig, 0, 15);?>...</td>
												<td><?=substr($row->linkedin, 0, 15);?>...</td>
												<td>
													<a href="<?=base_url('admin/edit_tutor/'.$row->id)?>"><button class="ui mini blue button"><i class="edit icon"></i></button></a>
													<a onclick="return confirm('Data akan dihapus!')" href="<?=base_url('admin/del_tutor/'.$row->id)?>"><button class="ui mini red button"><i class="trash icon"></i></button></a>
												</td>
											</tr>
										<?php } ?>
									</tbody>
